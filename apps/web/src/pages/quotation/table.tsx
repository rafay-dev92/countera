...
import type { Business, Customer, Paginated, Quotation as QuotationEntity } from "@/types/api";

type QuotationRow = QuotationEntity & { Business?: Business | null };
...
export function Quotation() {
    ...
    const [selectedQuotation, setSelectedQuotation] = useState<QuotationEntity | null>(null);
    ...
    const getQuotations = async () => {
        try {
            ...
            const totalQuotations: Paginated<QuotationRow> = await response.json();
    ...
    const createInvoice = async (quotationData: any) => {
        const confirmed = await confirm("Do you really want to create an invoice from this quotation?");
        if (!confirmed) return;
        setLoading(true)
        const selectedProductIds = quotationData?.Product?.map((product: any) => ({
            id: product.id,
