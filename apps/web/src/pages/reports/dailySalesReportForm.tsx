...
interface DailySalesReportFormValues {
    date: string;
}
...
function DailySalesReportForm({ open, close, setReportData, onReportGenerated }: DailySalesReportFormProps) {
    const { state } = State();
    ...
    const onSubmit = async (values: DailySalesReportFormValues) => {
        setIsLoading(true);
        try {
    ...
    const clearForm = (formikProps: ReturnType<typeof useFormik<DailySalesReportFormValues>>) => {
    ...
    const formikProps = useFormik<DailySalesReportFormValues>({
        initialValues: {
            date: '',
        },
