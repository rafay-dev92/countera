...
const schema = Yup.object().shape({
    ...
    image: Yup.mixed().notRequired().test(
        ...
        (value) => {
            ...
            return (
                !value ||
                (value && ['image/jpeg', 'image/png', 'image/gif'].includes((value as File).type.toLowerCase()))
            );
        }
    )
});
...
interface BusinessFormProps {
    ...
    open: boolean;
    close: () => void;
    selectedItem: Business | null;
    setSelectedItem: React.Dispatch<React.SetStateAction<Business | null>>;
}

const MyPopUpForm = ({ refresh, setRefresh, open, close, selectedItem, setSelectedItem }: BusinessFormProps) => {

    const { state } = State();
    const [isLoading, setIsLoading] = useState(false);
    const [edit, setEdit] = useState(false);
    const [businessPreviewPic, setBusinessPreviewPic] = useState<string | null>(null);
    ...
    useEffect(() => {
        if (selectedItem) {
            ...
            formikProps.setValues(selectedItem as unknown as BusinessFormValues);
            setEdit(true);
        }
    }, [selectedItem]);
    ...
    const onSubmit = async (values: BusinessFormValues) => {
        ...
        businessData.append('defaultMargin', values.defaultMargin as unknown as string);
    ...
    const clearForm = (formikProps: ReturnType<typeof useFormik<BusinessFormValues>>) => {
        formikProps.resetForm({
            values: {
                ...
                state: "",
                zipcode: "",
                image: businessPreviewPic as string
            },
            errors: {
                name: "",
                email: "",
                defaultMargin: 10 as unknown as string,
                tel: "",
                fax: "",
                licenseNumber: "",
                permitNumber: "",
    ...
    const formikProps = useFormik<BusinessFormValues>({
        initialValues: {
            name: "",
            email: "",
            defaultMargin: 10,
                                ...
                                <div className="flex items-center justify-start space-x-4 mb-3 w-full">
                                    ...
                                    <div className="basis-[25%]">
                                        ...
                                        <input
                                            ...
                                            onBlur={handleBlur}
                                        />
                                        {(touched as any).description && (errors as any).description && (
                                            <div className="mt-1 text-xs text-red-600">
