...
import type { Customer, CustomerVehicle, Inspection as InspectionRecord } from '@/types/api';

interface InspectionFormValues {
    customer: string;
    vehicle: string;
    data: typeof inspectionReport;
}
...
export function Inspection() {
    ...
    const [customerInspections, setCustomerInspections] = useState<InspectionRecord[]>([]);
    const [selectedInspection, setSelectedInspection] = useState<string | null>(null);
    const [customers, setCustomers] = useState<Customer[]>([]);
    const [selectedCustomer, setSelectedCustomer] = useState<Customer | null | undefined>(null);
    const [selectedVehicle, setSelectedVehicle] = useState<CustomerVehicle | null | undefined>(null);
    ...
    useEffect(() => {
        if (state.inspection?.selected) {
            ...
            setValues({ ['customer']: selected.CustomerId, ['vehicle']: selected.CustomerVehicleId!, data: JSON.parse(selected.data as string) });
            setShowCustomerSuggestions(false);
        }
    ...
    const handleCustomerChange = (customer: Customer) => {
        ...
        setCustomerInspections((customer as any).Inspection);
    ...
    useEffect(() => {
        if (selectedInspection) {
            const inspection = customerInspections.find((inspection) => inspection.id === selectedInspection);
            if (inspection) {
                const updatedInspection = { ...inspection, Customer: selectedCustomer } as InspectionRecord;
                dispatch({ type: 'SET_INSPECTION_DATA', payload: updatedInspection })
            }
        } else {
            dispatch({ type: 'SET_INSPECTION_DATA', payload: null })
    ...
    const clearForm = (formikProps: FormikProps<InspectionFormValues>) => {
    ...
    const formikProps = useFormik<InspectionFormValues>({
        initialValues: {
            customer: '',
            vehicle: '',
            data: inspectionReport
                                        ...
                                        <tr id='table' key={idx} className={`w-full border-b border-gray-200 px-4 py-1 text-xs ${item.category ? 'bg-slate-50 px-2 py-1.5 text-white py-2' : ''}`}>
                                            ...
                                            {!item.category ?
                                                <td className="border-b border-gray-200 px-4 py-1">
                                                    <span>{(values.data[idx] as any).good}</span>
