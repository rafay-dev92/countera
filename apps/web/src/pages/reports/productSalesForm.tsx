...
interface ProductSalesFormValues {
    product: string;
}
...
function ProductSalesForm({ open, close, setReportData }: ProductSalesFormProps) {
    ...
    const onSubmit = async (values: ProductSalesFormValues) => {
        setIsLoading(true);
        try {
    ...
    const handleProductChange = (product: Product) => {
        ...
        if (product) {
            setValues({ ['product']: product.id } as any);
        };
    ...
    const clearForm = (formikProps: ReturnType<typeof useFormik<ProductSalesFormValues>>) => {
    ...
    const formikProps = useFormik<ProductSalesFormValues>({
        initialValues: {
            product: '',
        },
                                ...
                                <div className="w-[25vw] p-6 space-y-4">
                                    <div className="relative flex flex-col" ref={productInputRef}>
                                        ...
                                        <input
                                            ...
                                            onClick={() => { setShowProductSuggestions(true); setValues({ ['product']: '' } as any) }}
