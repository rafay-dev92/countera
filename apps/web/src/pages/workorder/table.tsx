...
import type { Business, Customer, Paginated, WorkOrder as WorkOrderEntity } from "@/types/api";

type WorkOrderRow = WorkOrderEntity & { Business?: Business | null };
...
export function WorkOrder() {
    ...
    const [selectedWorkOrder, setSelectetWorkOrder] = useState<WorkOrderEntity | null>(null);
    ...
    const getWorkOrders = async () => {
        try {
            const filters: { status: string[]; CustomerDetails?: { name: string } } = {
            ...
            const totalWorkOrders: Paginated<WorkOrderRow> = await response.json();
    ...
    const handleEditWorkOrder = (index: number) => {
        const selected = workOrders[index];
        setSelectetWorkOrder(selected);
        openPopup();
    };

    const handleDeleteWorkOrder = async (index: number) => {
    ...
    const createInvoice = async (workOrderData: any) => {
        const confirmed = await confirm("Are you sure you want to create an invoice for this work order?");
        if (!confirmed) return;
        setLoading(true);
        const selectedProductIds = workOrderData?.Product?.map((product: any) => ({
            id: product.id,
                    ...
                    <div className="flex flex-col lg:flex-row items-center w-full mt-5">
                        <div className="w-full lg:w-2/5 flex items-center justify-center lg:justify-start gap-2">
                            ...
                            <div className="flex gap-2 lg:gap-4">
                                <Button disabled={!state.userInfo?.Permission?.includes("workorder:create")} className={`w-full bg-teal-700 hover:bg-teal-800 lg:w-auto`} size="md" onClick={openPopup} >
                                    New
                                </Button>
                            </div>
                        </div>
                    ...
                    <table className=" w-full min-w-max table-auto text-left">
                        <thead>
                            <tr>
                                ...
                                {(state.userInfo.role as string) === 'super_admin' && (
                                    <th
                                        key={'BUSINESS'}
                                        className="border-y border-blue-gray-100 bg-blue-gray-50/50 p-4"
                                    >
