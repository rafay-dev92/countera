...
export default function Permissions() {
    ...
    const [finalItems, setFinalItems] = useState<Permission[]>([]);
    ...
    const getPermissions = async () => {
        try {
            const permissions = (await (await fetchPermissions(state.userToken))!.json()) as Permission[];
    ...
    return (
        <>
            <Card className="h-full w-full ">
                <CardHeader floated={false} shadow={false} className="rounded-none">
                    <div className="mb-4 sm:mb-0 flex items-center">
                        <Typography variant="h5" color="blue-gray" className="flex items-center">
                            <ShieldCheckIcon className="h-12 w-12 text-blueGray-500 ml-2" />
                            Permissions
                        </Typography>
                    </div>
                ...
                <CardBody className="p-2 overflow-scroll px-0">
                    <table className=" w-full min-w-max table-auto text-left">
                        ...
                        <tbody>
                            {currentItems.map(({ id, name, description }, index) => {
            ...
            <PermissionForm refresh={refresh} setRefresh={setRefresh} open={isOpen} close={closePopup} selectedItem={selectedItem} setSelectedItem={setSelectedItem} />
        </>
    );
}