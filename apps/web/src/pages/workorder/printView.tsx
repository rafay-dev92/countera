...
interface PrintViewProps {
    view: boolean;
    workOrderData: any;
    appliedTaxes: Record<string, any>;
    labour?: any;
}

const printView = React.forwardRef<HTMLDivElement, PrintViewProps>(({ view, workOrderData, appliedTaxes }, ref) => {
    ...
    useEffect(() => {
        ...
        const updatedTaxes: any[] = [];
        workOrderData.Product.forEach((prod: any) => {
            if (workOrderData.Customer.taxable) {
                prod.Tax?.forEach((productTax: any) => {
                    if (!updatedTaxes.some((tax: any) => tax.id === productTax.id)) {
                ...
                <div className="w-full text-sm">
                    <table className="w-full min-w-max table-auto text-left">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200 text-xs">
                            {workOrderData.Product.map((item: any, index: number) => (
                                <tr key={index}>
                    ...
                    <div className="basis-[50%] max-w-[50%]">
                        ...
                        <div className="border-t border-x divide-y text-xs">
                            <div className="flex justify-between px-2 py-1">
                                ...
                                <span className="">${workOrderData?.labour}</span>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col">
                            ...
                            <div className="quill-content"
                                dangerouslySetInnerHTML={{
                                    __html: workOrderData?.Business.termsAndConditions || "",
                                }}
                            >
                            </div>
                        </div>
