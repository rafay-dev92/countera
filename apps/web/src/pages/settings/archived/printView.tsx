...
import type { ArchivedInvoice, Business, Customer, CustomerVehicle, Product, Tax } from "@/types/api";

/** Product row as the archived-invoice routes emit it (archived_invoice_product / Tax aliases). */
export type ArchivedInvoiceProduct = Product & {
    archived_invoice_product: { price: number; quantity: number; description?: string | null };
    Tax?: Tax[];
};

/** Archived invoice as printed: the route also joins Business/Customer/CustomerVehicle. */
export type ArchivedPrintInvoice = Omit<ArchivedInvoice, "Customer" | "CustomerVehicle" | "Products"> & {
    Business: Business;
    Customer: Customer;
    CustomerVehicle: CustomerVehicle;
    Products?: ArchivedInvoiceProduct[];
};

interface PrintViewProps {
    view: boolean;
    printInvoice: ArchivedPrintInvoice;
    appliedTaxes: Record<string, number>;
}

const printView = React.forwardRef<HTMLDivElement, PrintViewProps>(({ view, printInvoice, appliedTaxes }, ref) => {

    const invoiceDate = new Date(printInvoice?.createdAt);

    const calculateAmount = (price: number | string, quantity: number | string) => {
        const unitPrice = parseFloat(price as string) || 0;
        const qty = parseFloat(quantity as string) || 0;
        return (unitPrice * qty).toFixed(2);
    };

    const calculateTotalAmount = (products: ArchivedInvoiceProduct[] | undefined) => {
        let total = 0;
                    ...
                    <div className="basis-[50%] max-w-[50%]">
                        ...
                        <div className="border-t border-x divide-y text-xs">
                            <div className="flex justify-between px-2 py-1">
                                ...
                                <span className="">${parseFloat(printInvoice?.totalAmount.toFixed(2)) + parseFloat(printInvoice?.discount as unknown as string)}</span>
                            </div>
                        </div>
                        ...
                        {JSON.parse(printInvoice?.payments).length > 0 ? (
                            JSON.parse(printInvoice?.payments).map((payment: any, index: number) => {
                                ...
                                <div className="flex flex-wrap gap-3 text-[9px]">
                                    {[
                                        ...
                                        <label key={key} className="flex items-center gap-1">
                                            <input
                                                type="checkbox"
                                                checked={!!(printInvoice as any)?.[key] || false}
                                                readOnly
                                                className="w-3 h-3"
                                            />
                                            <span>{label}</span>
