...
const PaymentForm = ({ open, close, totalAmount, totalAmountPaid, invoiceId, setPrintInvoice }: PaymentFormProps) => {
    const { state } = State();
    ...
    const onSubmit = async (values: PaymentFormValues) => {
        setIsLoading(true);
        const data = { ...values, totalAmount: parseFloat(totalAmount as any), InvoiceId: invoiceId };
        try {
            const res = (await addPayment(data as any, state.userToken))!;
    ...
    const formikProps = useFormik<PaymentFormValues>({
        initialValues: {
            paymentMethod: "Card",
            paidAmount: "",
            cardNumber: "",
    ...
    return (
        <>
            <Dialog size='xs' open={open} handler={undefined as any} >
                {open && (
                                        ...
                                        <div className="">
                                            ...
                                            <input
                                                ...
                                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                                                    ...
                                                    const value =
                                                        Math.min(maxValue, (parseInt(e.target.value, 10) || "") as any) === 0
                                                            ? ""
                                                            : Math.min(maxValue, (parseInt(e.target.value, 10) || "") as any);
