...
// NOTE: pre-existing bug (left untouched): `business` is never defined in this module,
// so the localStorage fallback branch below would throw a ReferenceError at runtime.
// The ambient declaration only satisfies the typechecker without changing behavior.
declare const business: unknown;
              ...
              {routes.map(
                ({ layout, pages }) =>
                  ...
                  pages.map(({ path, element }) => (
                    <Route {...({ exact: true } as any)} path={path} element={element} />
                  ))
              )}
            </Routes>
          </div>
