...
interface MonthlyReportFormValues {
    month: string;
}
...
function MonthlyReportForm({ open, close, setReportData, onReportGenerated }: MonthlyReportFormProps) {
    const { state } = State();
    ...
    const onSubmit = async (values: MonthlyReportFormValues) => {
        setIsLoading(true);
        try {
    ...
    const clearForm = (formikProps: ReturnType<typeof useFormik<MonthlyReportFormValues>>) => {
    ...
    const formikProps = useFormik<MonthlyReportFormValues>({
        initialValues: {
            month: '',
        },
