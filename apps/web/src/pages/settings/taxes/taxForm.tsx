...
interface TaxFormProps {
    taxData: Tax | null;
    setTaxData: React.Dispatch<React.SetStateAction<Tax | null>>;
    open: boolean;
    close: () => void;
    refresh: boolean;
...
function TaxForm({ taxData, setTaxData, open, close, refresh, setRefresh }: TaxFormProps) {
    ...
    const onSubmit = async (values: TaxFormValues) => {
        ...
        try {
            if (!edit) {
                // saving tax data
                const res = (await addTax(updatedData as unknown as Parameters<typeof addTax>[0], state.userToken))!;
                ...
                const res = (await updateTax(taxData!.id, updatedData as unknown as Parameters<typeof updateTax>[1], state.userToken))!;
                const tax = await res.json();
                if (res.status === 200) {
                    toast.success(tax.message)
                }
    ...
    const clearForm = (formikProps: ReturnType<typeof useFormik<TaxFormValues>>) => {
    ...
    const formikProps = useFormik<TaxFormValues>({
        initialValues: {
            name: "",
            rate: "",
            type: "",
