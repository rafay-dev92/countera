...
import type { ArchivedInvoiceProduct, ArchivedPrintInvoice } from "./printView";
...
function Archived() {
    ...
    const [invoices, setInvoices] = useState<ArchivedPrintInvoice[]>([]);
    const [refresh, setRefresh] = useState(false);
    const [appliedTaxes, setAppliedTaxes] = useState<Record<string, number>>({});

    // for edit of a vehicle
    const [selectedItem, setSelectedItem] = useState<ArchivedPrintInvoice | null>(null);
    ...
    const getArchivedInvoices = async () => {
        try {
            ...
            const invoices: Paginated<ArchivedPrintInvoice> = await res.json();
    ...
    const handleEditInvoice = async (index: number) => {
        // setLoading(true);
    ...
    const recalculateTaxes = (products: ArchivedInvoiceProduct[], selectedCustomer: Customer | undefined) => {
    ...
    let filteredRows: ArchivedPrintInvoice[] = [];
                                ...
                                return (
                                    <tr key={id}>
                                        ...
                                        <td className={classes}>
                                            <Typography
                                                variant="small"
                                                color={(invoiceStatusColors[paymentStatus] || "gray") as any}
