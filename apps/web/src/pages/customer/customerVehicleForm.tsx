...
import type { CustomerVehicle, Vehicle } from "@/types/api";
...
const CustomerVehicleForm = ({ open, close, refresh, setRefresh, CustomerId, getCustomerDetails, selectedVehicle }: CustomerVehicleFormProps) => {
    const makeInputRef = useRef<HTMLDivElement | null>(null);
    const modelInputRef = useRef<HTMLDivElement | null>(null);
    const { state } = State();
    ...
    const years: number[] = [];
    for (let year = endYear; year >= (startYear as any); year--) {
        years.push(year);
    }
    ...
    const onSubmit = async (values: VehicleFormValues) => {
        ...
        try {
            if (!edit) {
                values = { ...values, CustomerId } as any
                const res = (await addCustomerVehicle(values as any, state.userToken))!
                ...
                const res = (await updateCustomerVehicle(selectedVehicle!.id, values as any, state.userToken))!
                const vehicle = await res.json();
                if (res.status === 200) {
                    showToastMessage('success', vehicle.message)
                }
    ...
    const clearForm = (formikProps: FormikProps<VehicleFormValues>) => {
    ...
    const formikProps = useFormik<VehicleFormValues>({
        initialValues: {
            year: "",
            make: "",
            model: "",
    ...
    return (
        <Dialog className="bg-transparent p-0" open={open} size="lg" handler={undefined as any}>
            {open && (
