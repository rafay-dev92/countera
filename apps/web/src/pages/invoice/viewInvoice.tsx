...
interface ViewInvoiceProps {
    /** Invoice payload; may be an empty object before an invoice is loaded. */
    printInvoice: any;
    setPrintInvoice: React.Dispatch<React.SetStateAction<any>>;
...
const ViewInvoice = ({ printInvoice, setPrintInvoice, componentRef, appliedTaxes, setEdit, close }: ViewInvoiceProps) => {
    ...
    const sendMailToUser = async () => {
        ...
        try {
            ...
            formData.append("businessName", state?.business?.name as any);
            formData.append("businessEmail", state?.business?.email as any);
    ...
    const renderAuditChange = (audit: any) => {
        const oldValue: any = JSON.parse(audit.oldValue as string);
        const newValue: any = JSON.parse(audit.newValue as string);
                ...
                return (
                    <div className="text-sm space-y-1">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            {oldValue[audit.fieldName]?.[field] && newValue[audit.fieldName]?.[field] && (
                                <div className="flex items-center gap-2">
                                    <span className="text-green-400">{(!oldValue[audit.fieldName]?.[field] as any) !== '' ? formatValue(oldValue[audit.fieldName][field]) : <span className="flex items-center gap-1"><XCircleIcon className="w-4 h-4" /> Empty</span>}</span>
                                    <ArrowLongRightIcon className="text-gray-400 w-4 h-4" />
                                    <span className="text-red-400">{(!newValue[audit.fieldName]?.[field] as any) !== '' ? formatValue(newValue[audit.fieldName][field]) : <span className="flex items-center gap-1"><XCircleIcon className="w-4 h-4" /> Empty</span>}</span>
                                </div>
                            )}
        ...
        const renderObjectChanges = (oldObj: any, newObj: any) => {
            return Object.entries(oldObj).map(([field, oldVal]: [string, any]) => {
            ...
            <div className="h-[70vh] lg:h-[78vh] overflow-hidden">
                ...
                <div className="hidden lg:flex h-full">
                    ...
                    <div className="flex-1 overflow-x-auto overflow-y-auto p-2">
                        <div className="max-h-[68vh] lg:max-h-[76vh] mx-auto">
