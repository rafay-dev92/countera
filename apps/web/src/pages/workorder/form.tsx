...
import type { Customer, CustomerVehicle, WorkOrder, Tax } from "@/types/api";
...
/** Dynamic line-item row for the products table (shape is API-driven and loose). */
interface LineItem {
  id?: string;
  product: string;
  name?: string;
...
interface MyPopUpFormProps {
  ...
  setSelectetWorkOrder: React.Dispatch<React.SetStateAction<WorkOrder | null>>;
}

const MyPopUpForm = ({ refresh, setRefresh, open, close, selectedWorkOrder, setSelectetWorkOrder }: MyPopUpFormProps) => {
  const componentRef = useRef<any>();
  const printRef = useRef<any>();
  const customerInputRef = useRef<any>();
  const productInputRef = useRef<any>();
  ...
  const [lumSum, setLumSum] = useState<any>(0);
  const [isLumSumApplied, setIsLumSumApplied] = useState(false);
  const [labour, setLabour] = useState<any>(0);
  const [labourBaseline, setLabourBaseline] = useState<LabourBaseline | null>(null);
  const [edit, setEdit] = useState(false);
  const [printWorkOrder, setPrintWorkOrder] = useState<WorkOrder | null>(null);
  const [appliedTaxes, setAppliedTaxes] = useState<Record<string, any>>({});
  const [showCustomerSuggestions, setShowCustomerSuggestions] = useState(false);
  const [showProductSuggestions, setShowProductSuggestions] = useState(false);
  const [vehicleOdometer, setVehicleOdometer] = useState<any>('');
  ...
  // workorder taxes
  const [workorderTaxes, setWorkorderTaxes] = useState<Record<string, any>>({});
  ...
  useEffect(() => {
    if (selectedWorkOrder) {
      ...
      setValues({ ...selectedWorkOrder, ['customer']: selectedWorkOrder.CustomerId, ['vehicle']: selectedWorkOrder.CustomerVehicleId } as any)
      setEdit(true)

      let selectedProd = [...selectedProducts]
      ;(selectedWorkOrder as any).Product?.forEach((prod: any) => {
        const aProd = {
          product: prod.id,
          id: prod.id,
          name: prod.name,
      ...
      selectedProd.forEach((product) => {
        product.Tax?.forEach((productTax) => {
          ...
          if (productTax.name === 'Sales Tax' && !selectedWorkOrder?.Customer?.taxable) {
      ...
      const parsedTaxes = JSON.parse(selectedWorkOrder.appliedTaxes as string) || {};
      setWorkorderTaxes(parsedTaxes);
      setAppliedTaxes(Object.values(parsedTaxes).reduce((acc: Record<string, any>, tax: any) => {
  ...
  const calculateLabour = (products: any[]) => {
    let totalLabour = 0;
  ...
  const distributeExcessToLabourProducts = (excessAmount: number) => {
    ...
    setSelectedProducts((prevProducts) => {
      ...
      ) || (state as any).workorderViewData?.labour || 0;
      ...
      if (labourIndexes.length === 0) {
        setLabour((prevLabour: any) => parseFloat(prevLabour) + excessAmount);
        return prevProducts;
      }
  ...
  const onSubmit = async (values: WorkOrderFormValues) => {
    ...
    const data = {
      workOrderData: {
        ...
        appliedTaxes: JSON.stringify(workorderTaxes),
      },
  ...
  const handleProductChange = async (index: number, quantity: any, selectedProId: any) => {
  ...
  const handleRemoveProduct = (index: number) => {
    ...
    let updatedWorkorderTaxes = { ...workorderTaxes };
    if (removedProduct) {
      removedProduct.Tax?.forEach((productTax) => {
        delete updatedWorkorderTaxes[`${removedProduct.id}_${productTax.id}`];
      });
    }

    if (updatedItems.length > 0) {
      recalculateTaxesWithUpdatedState(updatedItems, updatedWorkorderTaxes);
    } else {
      setAppliedTaxes({});
    }
  ...
  const recalculateTaxesWithUpdatedState = (products: LineItem[], updatedWorkorderTaxes: Record<string, any>) => {
    const tempWorkorderTaxes = updatedWorkorderTaxes || {};
    ...
    products.forEach((product) => {
      product.Tax?.forEach((productTax) => {
        ...
        if (!tempWorkorderTaxes[key2]) {
          tempWorkorderTaxes[key2] = {
            TaxId: productTax.id,
        ...
        if (productTax.type === "%") {
          tempWorkorderTaxes[key2].tax_amount = (product.price * product.quantity * (tempWorkorderTaxes[key2].tax_rate / 100));
        } else {
          tempWorkorderTaxes[key2].tax_amount = (product.quantity * tempWorkorderTaxes[key2].tax_rate);
        }
      });
    });

    setAppliedTaxes(Object.values(tempWorkorderTaxes).reduce((acc: Record<string, any>, tax: any) => {
      const key = `${tax.TaxId}_${tax.tax_rate}_${tax.tax_type}`;
    ...
    Object.values(tempWorkorderTaxes).forEach((tax: any) => {
  ...
  // Recalculate taxes
  const recalculateTaxes = (products: LineItem[]) => {
    const productTaxes: Record<string, number> = {};
    const tempWorkorderTaxes = workorderTaxes || {};
    ...
    // setAppliedTaxes(productTaxes);
    setAppliedTaxes(Object.values(workorderTaxes).reduce((acc: Record<string, any>, tax: any) => {
      const key = `${tax.TaxId}_${tax.tax_rate}_${tax.tax_type}`;
  ...
  const handleTaxableChange = (index: number, isChecked: any) => {
    const updatedItems = [...selectedProducts];
  ...
  const calculateTotalTaxAmount = () => {
    ...
    // }
    if (Object.values(workorderTaxes).length > 0) {
      Object.keys(workorderTaxes).forEach((tax) => {
        totalTaxAmount += parseFloat(workorderTaxes[tax].tax_amount);
      });
      return totalTaxAmount;
    }
    return 0;
  ...
  const clearForm = (formikProps: ReturnType<typeof useFormik<WorkOrderFormValues>>) => {
  ...
  const formikProps = useFormik<WorkOrderFormValues>({
    initialValues: {
      customer: "",
      vehicle: "",
      comments: "",
  ...
  const handleLumSum = () => {
    ...
    if (lump > currentTotal) {
      ...
      if ((difference as any) > maxDiscount) {
  ...
  return (
    <>
      <Dialog className="bg-transparent shadow-none p-0" open={open} size="xl" {...({} as any)}>
        {open && (
              ...
              <div className="bg-white rounded-xl shadow-xl overflow-hidden w-full">
                <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-4 py-2">
                  <div className="text-[15px] font-semibold text-slate-900">
                    {state?.workorder?.isViewOpen ? "View work order" : edit ? "Edit work order" : "New work order"}
                  </div>
                  <button
                    ...
                    <div className="flex flex-col lg:flex-row gap-4">
                      <div className="w-full lg:w-[35%]">
                        ...
                        <div className="space-y-4 px-2">
                          <div>
                            <label className="p-2 text-[13px] font-medium text-slate-700">Name</label> <br />
                            <input
                          ...
                          <div>
                            ...
                            <label className="p-2 text-[13px] font-medium text-slate-700">Email</label> <br />
                            <input
                              className="w-full h-9 rounded-md border border-slate-300 bg-white px-2.5 text-sm text-slate-900 focus:border-teal-600 focus:outline-none focus:ring-2 focus:ring-teal-600/20 disabled:bg-slate-50 disabled:text-slate-500"
                              id="email"
                              name="email"
                              type="email"
                              value={selectedCustomer ? (selectedCustomer.email as string) : ''}
                              disabled
                            />
                          </div>

                          <div>
                            <label className="p-2 text-[13px] font-medium text-slate-700">Phone</label> <br />
                            ...
                            <input
                              ...
                              name="phone"
                              type="text"
                              value={selectedCustomer ? (selectedCustomer.phone as string) : ''}
                              disabled
                            />
                          </div>

                          <div>
                            <label className="p-2 text-[13px] font-medium text-slate-700">Address</label> <br />
                        ...
                        <div className="">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="space-y-4">
                              <div>
                                <label className="p-2 text-[13px] font-medium text-slate-700">Make</label>
                                <div className="px-2">
                                  <input
                                    className="w-full h-9 rounded-md border border-slate-300 bg-white px-2.5 text-sm text-slate-900 focus:border-teal-600 focus:outline-none focus:ring-2 focus:ring-teal-600/20 disabled:bg-slate-50 disabled:text-slate-500"
                                    id="make"
                              ...
                              <div>
                                <label className="p-2 text-[13px] font-medium text-slate-700">Model</label>
                                <div className="px-2">
                                  <input
                                    className="w-full h-9 rounded-md border border-slate-300 bg-white px-2.5 text-sm text-slate-900 focus:border-teal-600 focus:outline-none focus:ring-2 focus:ring-teal-600/20 disabled:bg-slate-50 disabled:text-slate-500"
                                    id="model"
                            ...
                            <div className="space-y-4">
                              <div>
                                <label className="p-2 text-[13px] font-medium text-slate-700">Year</label>
                                <div className="px-2">
                                  <input
                                    className="w-full h-9 rounded-md border border-slate-300 bg-white px-2.5 text-sm text-slate-900 focus:border-teal-600 focus:outline-none focus:ring-2 focus:ring-teal-600/20 disabled:bg-slate-50 disabled:text-slate-500"
                                    id="year"
                              ...
                              <div>
                                <label className="p-2 text-[13px] font-medium text-slate-700">Color</label>
                                <div className="px-2">
                                  <input
                                    className="w-full h-9 rounded-md border border-slate-300 bg-white px-2.5 text-sm text-slate-900 focus:border-teal-600 focus:outline-none focus:ring-2 focus:ring-teal-600/20 disabled:bg-slate-50 disabled:text-slate-500"
                                    id="color"
                                    name="color"
                                    type="text"
                                    value={selectedVehicle ? (selectedVehicle.color as string) : ''}
                                    disabled
                                  />
                                </div>
                              </div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="space-y-4">
                              ...
                              <div>
                                <label className="p-2 text-[13px] font-medium text-slate-700">License No.</label>
                                <div className="px-2">
                                  <input
                                    className="w-full h-9 rounded-md border border-slate-300 bg-white px-2.5 text-sm text-slate-900 focus:border-teal-600 focus:outline-none focus:ring-2 focus:ring-teal-600/20 disabled:bg-slate-50 disabled:text-slate-500"
                                    id="color"
                                    name="color"
                                    type="text"
                                    value={selectedVehicle ? (selectedVehicle.licenseNo as string) : ''}
                                    disabled
                                  />
                                </div>
                              </div>
      ...
      <CustomerForm open={isCustomerFormOpen} close={closeCustomerForm} refresh={refresh} setRefresh={setRefresh} setSelectedCustomer={setSelectedCustomer} {...({} as any)} />
      {selectedCustomer ? <CustomerVehicleForm open={isCustomerVehicleFormOpen} close={closeCustomerVehicleForm} refresh={refresh} setRefresh={setRefresh} CustomerId={selectedCustomer?.id} getCustomerDetails={getCustomerDetails} {...({} as any)} /> : null}
          ...
          <div className="relative bg-white p-6 rounded-xl shadow-2xl z-[10000]
            ...
            <div className="mt-4">
              ...
              <input
                ...
                min={1}
                value={modalQuantity}
                onChange={(e) => (e.target.value as any) > 0 ? setModalQuantity(Number(e.target.value)) : setModalQuantity(e.target.value)}
