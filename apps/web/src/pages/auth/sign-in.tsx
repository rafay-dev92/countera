...
import type { AuthTokens, Business } from "@/types/api";

interface BusinessesForEmailResponse {
  businesses: Business[];
  isSuperAdmin?: boolean;
  message?: string;
}

type SignInResponse = AuthTokens & { message?: string };
...
export function SignIn() {
  ...
  const getUserInfo = async (token: string) => {
    try {
      const UserInfo = await ((await getUserDetails(token)) as Response).json();
      return UserInfo;
    } catch (error) {
      return null;
    }
  ...
  async function handleCheckEmail() {
    ...
    try {
      ...
      const data: BusinessesForEmailResponse = await res.json();
      setLoading(false);
