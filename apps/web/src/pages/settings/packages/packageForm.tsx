...
import type { Package, Product, Tax } from "@/types/api";
...
/** Product payload as the package routes actually emit it (Tax / package_product aliases). */
type PackageProduct = Product & {
    Tax?: Tax[];
    package_product: { quantity: number };
};

/** Row shown in the package's product table. */
...
function PackageForm({ packageData, setPackageData, open, close, refresh, setRefresh }: PackageFormProps) {
    const productInputRef = useRef<HTMLDivElement>(null);
    const { state } = State();
    const [isLoading, setIsLoading] = useState(false);
    const [edit, setEdit] = useState(false);
    const [products, setProducts] = useState<PackageProduct[]>([]);
    const [selectedProducts, setSelectedProducts] = useState<PackageLineItem[]>([{
        product: "",
        quantity: 1,
        price: 0,
        taxable: false
    ...
    useEffect(() => {
        if (packageData) {
            ...
            formikProps.setValues(data as PackageFormValues);
            let selectedProd = [...selectedProducts];
            const pkg = packageData as (Package & { Product?: PackageProduct[] }) | null;
            pkg?.Product?.forEach((prod) => {
                const aProd = {
                    product: prod.id,
                    id: prod.id,
                    name: prod.name,
    ...
    const getProducts = async () => {
        try {
            ...
            const productsData: PackageProduct[] = await fetchedProducts.json();
    ...
    const clearForm = (formikProps: ReturnType<typeof useFormik<PackageFormValues>>) => {
    ...
    const formikProps = useFormik<PackageFormValues>({
        initialValues: {
            name: "",
            // quantity: 1,
            description: "",
