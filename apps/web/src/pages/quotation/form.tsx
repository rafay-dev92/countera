...
import type { Customer, CustomerVehicle, Quotation, Tax } from "@/types/api";
...
const MyPopUpForm = ({ refresh, setRefresh, open, close, selectedQuotation, setSelectedQuotation }: MyPopUpFormProps) => {
  ...
  // quote taxes
  const [quoteTaxes, setQuoteTaxes] = useState<Record<string, any>>({});
  ...
  useEffect(() => {
    if (selectedQuotation) {
      ...
      setValues({ ...selectedQuotation, ['customer']: selectedQuotation.CustomerId, ['vehicle']: selectedQuotation.CustomerVehicleId } as any)
      setEdit(true)

      let selectedProd = [...selectedProducts];
      (selectedQuotation as any).Product?.forEach((prod: any) => {
        const aProd = {
          product: prod.id,
          id: prod.id,
          name: prod.name,
      ...
      const parsedTaxes = JSON.parse(selectedQuotation.appliedTaxes as string) || {};
  ...
  const distributeExcessToLabourProducts = (excessAmount: number) => {
    ...
    setSelectedProducts((prevProducts) => {
      ...
      ) || (state as any).quotationViewData?.labour || 0;
  ...
  const onSubmit = async (values: QuotationFormValues) => {
    ...
    if ((selectedVehicle?.odometer as any) < vehicleOdometer) {
      try {
  ...
  const recalculateTaxesWithUpdatedState = (products: LineItem[], updatedQuoteTaxes: Record<string, any>) => {
    ...
    setAppliedTaxes(Object.values(tempQuoteTaxes).reduce((acc: Record<string, any>, tax: any) => {
      const key = `${tax.TaxId}_${tax.tax_rate}_${tax.tax_type}`;
  ...
  const recalculateTaxes = (products: LineItem[]) => {
    ...
    setAppliedTaxes(Object.values(quoteTaxes).reduce((acc: Record<string, any>, tax: any) => {
      const key = `${tax.TaxId}_${tax.tax_rate}_${tax.tax_type}`;
  ...
  const clearForm = (formikProps: ReturnType<typeof useFormik<QuotationFormValues>>) => {
  ...
  const formikProps = useFormik<QuotationFormValues>({
    initialValues: {
      customer: "",
      vehicle: "",
      comments: "",
