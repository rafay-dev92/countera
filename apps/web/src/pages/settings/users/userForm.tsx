...
import type { ApiUser, Permission as ApiPermission } from "@/types/api";
...
interface UserFormProps {
    open: boolean;
    close: () => void;
    selectedItem: ApiUser | null;
    setSelectedItem: React.Dispatch<React.SetStateAction<ApiUser | null>>;
    refresh: boolean;
    setRefresh: React.Dispatch<React.SetStateAction<boolean>>;
}

const UserForm = ({ open, close, selectedItem, setSelectedItem, refresh, setRefresh }: UserFormProps) => {

    const { state } = State();
    const [isLoading, setIsLoading] = useState(false);
    const [edit, setEdit] = useState(false);
    const [permissions, setPermissions] = useState<ApiPermission[]>([]);
    const [selectPerms, setSelectPerms] = useState<string[]>([]);
    ...
    const getPermissions = async () => {
        try {
            ...
            setPermissions((await res.json()) as ApiPermission[])
        } catch (error) {
    ...
    useEffect(() => {
        if (selectedItem) {
            const { password, Permission, BusinessId, ...rest } = selectedItem as ApiUser & { password?: string };
            formikProps.setValues(rest as unknown as UserFormValues);
    ...
    const onSubmit = async (values: UserFormValues) => {
        ...
        if (values.dob === "") {
            delete (updatedValues as { dob?: string }).dob;
        }
        const userData = {
            user: updatedValues,
    ...
    const handlePermission = (id: string, e: { target: { checked: boolean } }) => {
    ...
    const clearForm = (formikProps: ReturnType<typeof useFormik<UserFormValues>>) => {
    ...
    const formikProps = useFormik<UserFormValues>({
        initialValues: {
            first_name: "",
            last_name: "",
            email: "",
    ...
    const groupedPermissions = permissions.reduce<Record<string, Array<ApiPermission & { action: string }>>>((acc, perm) => {
    ...
    return (
        <>
            <Dialog open={open} handler={undefined as any}>
                {open && (
                                    ...
                                    <div className="w-full mt-2">
                                        <div className="flex items-center gap-1 my-2">
                                            <input
                                                ...
                                                value={(selectPerms?.length == permissions?.length) as any}
