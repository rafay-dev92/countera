...
export function Appointments() {
    ...
    const getAppointments = async () => {
        try {
            ...
            if (res.status === 200) {
                ...
                showToastMessage('error', appointments.error)
                setLoading(false);
            }
        } catch (error) {
            console.log(error);
    ...
    const handleEdit = (data: Appointment) => {
    ...
    const isCurrentMonth = ((parseInt(monthList.indexOf(selected.split(' ')[0]) as any) + 1 > (new Date()).getMonth() + 1) || (parseInt(selected.split(' ')[1]) > (new Date()).getFullYear()))
