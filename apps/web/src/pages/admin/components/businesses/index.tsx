...
export default function Businesses() {
    ...
    const [finalItems, setFinalItems] = useState<Business[]>([]);
    ...
    const getBusinesses = async () => {
        try {
            const businesses = (await (await fetchBusinesses(state.userToken))!.json()) as Business[];
            setFinalItems(businesses);
            setLoading(false);
        } catch (error) {
            ...
            <Card className="h-full w-full ">
                ...
                <CardBody className="p-2 overflow-scroll px-0">
                    <table className=" w-full min-w-max table-auto text-left">
                        ...
                        <tbody>
                            {currentItems.map(({ id, name, email, address, city, state, tel, licenseNumber }, index) => {
